...
- **Functional requirements:** section **“## 2) Functional requirements”** (FR-1 … FR-7).
...
- Use a clear **project title** and **short description** referencing the feature: planner to-do bucketing with preference for “before 10 PM local → previous day” behavior (aligned with `agents/tasks/feature-1/feature-1.md`).
...
|----------|----------------|---------------------------|
| **Functional** | FR-1 … FR-7 | One issue per FR **or** one issue per FR group with checklists (must preserve AC traceability to FR IDs in the body). |
...
- Set **Priority** (or the project’s priority field) using a consistent scale, for example:
  - **P1 — High:** Core functional stories (FR-1, FR-2, FR-3, FR-5, FR-6) and primary unit-test coverage.
...
- [ ] **Handoff coverage:** Every **FR-1 … FR-7** appears as an explicit checkbox, issue, or linked sub-issue; **AC-1 … AC-6** are reflected in testing or functional issues.
...
From `agents/tasks/feature-1/feature-1.md` / implementation research: implement a **better Planner to-do experience** so that, when a user **prefers** it, items due **before 10:00 PM local** appear in the **previous day’s** bucket; **exactly 10:00 PM** stays same-day; disabled/missing preference preserves current behavior; timezone and observer-mode behavior must remain correct per FR/NFR and open questions.
